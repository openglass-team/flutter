...
// 环形缓冲区 (PSRAM 分配)
#define BUF_SZ 16384  // 16K 采样 = 32KB = ~1 秒缓冲
...
// ---- TCP 音频服务器 ----
WiFiServer tcpAinServer(8083);
WiFiClient  tcpAinClient;
WiFiServer tcpMicServer(8082);
WiFiClient  tcpMicClient;

// ---- HTTP 服务器 (端口 82, 避免与 WS :81 冲突) ----
WebServer server(82);
...
void handlePcmUpload() {
  ...
  if (u.status == UPLOAD_FILE_START) { s_pcm_bytes = 0; }
    ...
    audio_push(u.buf, u.currentSize); s_pcm_bytes += u.currentSize;
  }
  else if (u.status == UPLOAD_FILE_END) {
    Serial.printf("PCM HTTP: %d B\n", (int)s_pcm_bytes);
  }
}
...
void handleStatus() {
  ...
  snprintf(js, sizeof(js),
    "{\"buf_avail\":%d,\"buf_cap\":%d,\"underrun\":%d,\"overflow\":%d}",
    (int)buf_avail_read(), BUF_SZ, (int)s_underrun_count, (int)s_overflow_count);
...
void handleRoot() {
  server.send(200, "text/html",
    ...
    "<h1>OpenGlass Audio</h1><p>HTTP :82 | TCP :8083 (speaker) | TCP :8082 (mic)</p></body></html>");
}
...
void setup() {
  ...
  if (speaker.begin(spk_cfg)) {
    Serial.println("Speaker OK (I2S1)");
    speaker.unmute();
  ...
  tcpAinServer.begin();
  tcpMicServer.begin();
  Serial.println("HTTP :82 | TCP ain :8083 | TCP mic :8082 | WS :81 | TCP :8080 | TCP :8081");
...
void loop() {
  ...
  if (tcpAinServer.hasClient()) {
    ...
    } else { WiFiClient r = tcpAinServer.available(); r.stop(); }
  }
  ...
  if (tcpMicServer.hasClient()) {
    ...
    } else { WiFiClient r = tcpMicServer.available(); r.stop(); }
  }
  if (tcpMicClient && !tcpMicClient.connected()) tcpMicClient.stop();
